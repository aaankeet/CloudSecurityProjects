...
# Allow IncidentResponder1 to assume IncidentResponderRole.
# Note: The user must authenticate with MFA. The MFA session context is forwarded
# when IncidentResponderRole subsequently assumes the workload IncidentResponseRole,
# satisfying the aws:MultiFactorAuthPresent condition on that role's trust policy.
resource "aws_iam_user_policy" "incident_responder1_assume_policy" {
  ...
  policy = jsonencode({
    ...
    Statement = [{
      ...
      Condition = {
        Bool = {
          # Enforce MFA at the point of assuming the security account role
          # so that MFA context is present for the subsequent workload assumption.
